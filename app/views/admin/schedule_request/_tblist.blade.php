    ...
    <div class="row form-inline tblist-form-toolbar" >
        <div class="col-sm-9">
            <div class="form-group">
                <input type="text" name="student_id" class="form-control" placeholder="{{ lang('student/attributes.placeholders.user_id') }}" value="{{ Input::get('user_id') }}" style="width:180px">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <select name="semester" class="form-control">
                    ...
                    @foreach (get_semesters() as $key=>$value)
                        <option value="{{ $key }}" {{ is_selected($key, Input::get('semester')) }}>{{ $value }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <select name="status" class="form-control">
                    ...
                    @foreach (StudentPlotting::$statuses as $key=>$value)
                        <option value="{{ $key }}" {{ is_selected($key, Input::get('status')) }}>{{ $value }}</option>
                    @endforeach
                </select>
            </div>
        </div>
