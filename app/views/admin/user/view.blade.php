...
@include('admin._partials._messages')
    ...
    <div class="row">
        <div class="col-md-4">
            <h3>{{ trans('user::texts.details') }}</h3>
            <p>{{ $user->userDetail->getAddress() }}</p>
            <p>{{ $user->userDetail->getPhone() }}</p>
        </div>
        <div class="col-md-5">
            <h3>{{ trans('user::texts.contacts') }}</h3>
            {{ $user->present()->getContactDetails() }}
        </div>
    </div>
